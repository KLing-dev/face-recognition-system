...
### 2. 人脸分割（ModNet）
- 功能：对检测到的人脸区域进行精细化分割，提取高质量的人脸掩码
- 优势：轻量级、高效，支持实时分割
- 流程：人脸图像 → 预处理 → ModNet预训练模型 → 人脸掩码 → 应用掩码到原始图像
- 优化：增加了边缘平滑处理，提高分割质量

### 3. 人脸特征提取（FaceNet）
- 功能：将分割后的人脸图像转换为128维的特征向量
- 优势：特征向量具有良好的区分性，支持相似度计算
- 流程：分割后的人脸图像 → 预处理（归一化、对齐） → FaceNet预训练模型 → 128维特征向量
